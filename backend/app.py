...
def send_bulk():
    ...
    sender_pass = creds.get("app_password")
    ...
    if not recipients:
        return jsonify({"success": False, "message": "No valid email addresses found"}), 400
    ...
    for email in invalid_recipients:
        results[email] = "failed ❌ (invalid email)"
        failed_emails.append(email)
        failed_count += 1

    server.quit()

    # 7️⃣ Save results to PostgreSQL
    try:
        ...
        for email, status_msg in results.items():
            status = "sent" if status_msg.startswith("sent") else "failed"
